...
def oblast_row(region: dict) -> str:
    ...
    # Период у всех регионов один и назван в шапке блока: повторять его двадцать
    # раз значит забить строку тем, что не различает регионы.
    ...
    return (
        ...
        f"{f'<span>{stale.strip(chr(32) + chr(183))}</span>' if stale else ''}</span>"
        f'{middle}<span class="obl-figures">{figures}</span></div>'
    )


def oblast_section(data: dict | None) -> str:
    """Доходы бюджетов регионов из статистического бюллетеня Минфина.

    Раньше здесь был список тех, кто успел отчитаться у себя на сайте: периоды
    расходились на годы, и порядок по сумме ничего не значил. Бюллетень даёт все
    двадцать регионов за один и тот же период, поэтому это уже рейтинг.
    """
    regions = (data or {}).get("regions") or []
    if not regions:
        return ""
    period = period_label(regions[0]["year"], regions[0]["months"])
    return "\n".join(
        [
            "<h3>Доходы регионов</h3>",
            '<p class="lede">Все двадцать регионов за один период, '
            f"{period}. Полоса показывает, какую долю доходов регион собирает сам, "
            "а какую получает трансфертами из республиканского бюджета. Процент "
            "рядом это исполнение плана поступлений.</p>",
            f'<div class="obl">{"".join(oblast_row(r) for r in regions)}</div>',
            '<p class="obl-legend"><span><i class="own"></i>свои доходы</span>'
            '<span><i class="transfer"></i>трансферты</span></p>',
            '<p class="obl-note">Источник: статистический бюллетень Министерства '
            "финансов, отдельный лист исполнения бюджета по каждому региону. Здесь "
            "бюджет региона целиком, вместе с районными: областной бюджет без "
            "районов был бы заметно меньше.</p>",
        ]
    )
