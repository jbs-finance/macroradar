...
`Content-Security-Policy: script-src 'none'` в `build_macroradar.py`: раз
скрипт странице не нужен, поэтому его явно запрещают: лазейки не остаётся.
...
выгрузке правда достаточно точек: пустой массив гейт не устроит). Обе валят
