        ...
        rows.append(
            {
                ...
                "obs": [
                    {"date": "2024", "value": 10 + index},
                    {"date": "2025", "value": 11 + index},
                ],
                "stale": False,
...
def test_hub_links_to_all_topic_pages_with_own_urls():
    document = documents()[""]
...
def test_hub_respects_reduced_motion():
    ...
    assert (
        ".radar-card:hover, .radar-card:focus-visible { transform: none; }" in document
    )
